...
/** Check magic bytes in a Buffer (used with multer memoryStorage). */
export function isValidImageBuffer(buffer: Buffer): boolean {
  if (buffer.length < 12) return false;
  return checkHeader(buffer);
}

/** Check magic bytes by reading the first 12 bytes of a file on disk. */
...
function checkHeader(header: Buffer): boolean {
  // JPEG: FF D8 FF
